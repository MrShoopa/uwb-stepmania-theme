...
local EXF_Metertype={};
EXF_Metertype=getenv("EXF_Metertype");
local sgSys=getenv("sgSys");
local grSys=getenv("grSys");
		...
		SetCommand=function(self)
			...
			for chk=0,4 do
				if (wheelmode == 'Jacket->Banner' and GetSongGPath_JBN(songs[((st_index+#songs-2)+chk)%#songs+1])==g)
					...
					or (wheelmode == 'BG->Jacket' and GetSongGPath_BGJ(songs[((st_index+#songs-2)+chk)%#songs+1])==g) 
