	...
	if GAMESTATE:IsPlayerEnabled(pn) then
		...
		t[#t+1]=Def.ActorFrame{
			...
			LoadActor(THEME:GetPathG("_objects/_circle","glow100px"))..{
				...
				SetCommand=function(self)
					GetNetJud(pn);
--	_SYS("W1."..eva_w1.."/W2."..eva_w2.."/W3."..eva_w3.."/W4."..eva_w4.."/W5."..eva_w5.."/MS."..eva_ms);
...
for pn in ivalues(PlayerNumber) do
	if GAMESTATE:IsPlayerEnabled(pn) then
		...
		cal[p]=GetUserPref_Theme("UserShowCalories"..nm);
		if not cal[p] then cal[p]='Off' end;
	end;
end;
-- [ja] カロリー
local cal_prof={};
local cal_total={0.0,0.0};
if cal[1]=='On' then
	t[#t+1]=Def.ActorFrame{
		LoadFont("Common Normal")..{
			InitCommand=function(self)
			...
			OnCommand=function(self)
				cal_prof[1]=PROFILEMAN:GetProfile(PLAYER_1);
				cal_total[1]=cal_prof[1]:GetCaloriesBurnedToday();
				self:settextf("%.2f kcal\n%.2f kcal", getenv("cal1"), cal_total[1]);
			end;
		};
	};
end;
if cal[2]=='On' then
	t[#t+1]=Def.ActorFrame{
		LoadFont("Common Normal")..{
			...
			OnCommand=function(self)
				cal_prof[2]=PROFILEMAN:GetProfile(PLAYER_2);
				cal_total[2]=cal_prof[2]:GetCaloriesBurnedToday();
				self:settextf("%.2f kcal\n%.2f kcal", getenv("cal2"), cal_total[2]);
			end;
		};
	};
end;
...
t[#t+1] = Def.ActorFrame {
	...
	Def.Sprite{
		OnCommand=function(self)
			if _SONG():HasCDTitle() then
				if FILEMAN:DoesFileExist(_SONG():GetCDTitlePath()) then
					self:LoadBackground(_SONG():GetCDTitlePath());
					self:visible(true);
				end;
				(cmd(horizalign,right;vertalign,bottom;))(self);
				self:visible(showtitleFlag and (not GAMESTATE:IsCourseMode()));
			else
				self:visible(false);
			end;
			self:queuecommand("SwitchSongTitle");
		end;
		SwitchSongTitleCommand=function(self)
			if showtitleFlag and (not GAMESTATE:IsCourseMode()) then
				self:zoom(0.75);
...
t[#t+1] = LoadFont("Common Normal")..{
	InitCommand=function(self)
		...
		self:y(SCREEN_TOP+58);
		self:settextf("%s - %s%s",string.upper(GAMESTATE:GetCurrentGame():GetName()),string.upper(GAMESTATE:GetCurrentStyle():GetName())
			,IsUltimateLife() and " - ULTIMATELIFE" or "");
	end;
};
t[#t+1] = Def.ActorFrame {
