...
local mx=SCREEN_CENTER_X;
local my=SCREEN_CENTER_Y;
local function iCallback(e)
	local btn=ToEnumShortString(e.DeviceInput.button);
	local typ=ToEnumShortString(e.type);
	if btn=='left mouse button' and typ=="FirstPress" then
		MESSAGEMAN:Broadcast('Point');
	end;
end;
local function update(self)
	mx = INPUTFILTER:GetMouseX();
	my = INPUTFILTER:GetMouseY();
end;
t[#t+1]=Def.ActorFrame{
	OnCommand=function(self)
		self:SetUpdateFunction(update);
		if GetSMVersion()>=40 then
			SCREENMAN:GetTopScreen():AddInputCallback(iCallback);
		end;
	end;
	Def.Quad{
		InitCommand=cmd(zoomto,80,80;diffuse,Color("Blue");diffusealpha,0;blend,'BlendMode_Add');
		PointMessageCommand=cmd(finishtweening;
			x,mx;y,my;zoomto,0,0;rotationz,0;
			linear,0.15;zoomto,80,80;diffusealpha,1;rotationz,90;
			linear,0.15;zoomto,160,160;diffusealpha,0;rotationz,180;);
	};
};
