...
t[#t+1]=Def.ActorFrame{
	...
	LoadFont("Common normal")..{
		InitCommand=cmd(horizalign,(p==1) and left or right;
			vertalign,bottom;x,(p==1) and -120 or 120;y,-2;
			zoom,0.55;maxwidth,90/0.55;);
		Set2Command=function(self)
			if st and song then
	...
	LoadFont("waiei2 Combo Numbers")..{
		InitCommand=cmd(horizalign,(p==2) and right or left;
			vertalign,top;x,(p==1) and -120 or 120;y,6;zoom,0.5;
			zoom,0.6;maxwidth,90/0.6;);
		Set2Command=function(self)
			if st and song then
				self:diffuse(_DifficultyLightCOLOR2(difcolor,dif));
				self:strokecolor(ColorDarkTone(_DifficultyCOLOR2(difcolor,dif)));
