...
return Def.ActorFrame {
	...
	LoadFont("Common Normal") .. {
		InitCommand=function(self)
			if string.find(gc:GetText(),'^Custom1$') then
				self:settext(split(',',TC_GetOther('CustomMenu','Title'))[1]);
			elseif string.find(gc:GetText(),'^Custom2$') then
				self:settext(split(',',TC_GetOther('CustomMenu','Title'))[2]);
			elseif string.find(gc:GetText(),'^Custom3$') then
				self:settext(split(',',TC_GetOther('CustomMenu','Title'))[3]);
			elseif string.find(gc:GetText(),'^Custom4$') then
				self:settext(split(',',TC_GetOther('CustomMenu','Title'))[4]);
			else
				self:settext(THEME:GetString("ScreenTitleMenu",gc:GetText()));
			end;
		end;
